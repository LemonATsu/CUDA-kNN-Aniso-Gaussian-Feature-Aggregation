...
if __name__ == '__main__':
    ...
    for i in range(N):
        ...
        f_out_sum = f_out.sum(dim=-1)
        grad_w_q = torch.autograd.grad(
            outputs=f_out_sum,
            inputs=q,
            grad_outputs=torch.ones_like(f_out_sum),
            create_graph=True,
        )[0]
        ((grad_w_q[..., 0] ).pow(2.).mean()).backward()
        ...
        start = time.time()
        f_out_torch, w_torch, w_all, dist_torch, idxs_torch = torch_aggregate(q_torch, p_torch, f_torch, sigma_torch, R, K)
        f_out_torch_sum = f_out_torch.sum(dim=-1)
        grad_w_q_torch = torch.autograd.grad(
            outputs=f_out_torch_sum,
            inputs=q_torch,
            grad_outputs=torch.ones_like(f_out_torch_sum),
            create_graph=True,
        )[0]
        ((grad_w_q_torch[..., 0] ).pow(2.).mean()).backward()
